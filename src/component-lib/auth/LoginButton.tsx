import { Loader2 } from 'lucide-react';

export interface LoginButtonProps {
  onClick: () => Promise<void> | void;
  isLoading?: boolean;
}

function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

export function LoginButton({ onClick, isLoading = false }: LoginButtonProps) {
  return (
    <button
      type="button"
      onClick={() => void onClick()}
      disabled={isLoading}
      className="group relative flex items-center gap-3 rounded-2xl border border-stone-200 bg-white px-6 py-4 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-stone-200/60 active:translate-y-0 active:shadow-sm disabled:pointer-events-none disabled:opacity-60"
      aria-label="Sign in with Google"
    >
      <span className="absolute inset-0 rounded-2xl bg-gradient-to-r from-blue-500/5 via-red-500/5 to-yellow-500/5 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      <span className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-stone-50 transition-colors group-hover:bg-white">
        {isLoading ? (
          <Loader2 className="h-5 w-5 animate-spin text-stone-400" />
        ) : (
          <GoogleIcon className="h-5 w-5" />
        )}
      </span>
      <span className="relative flex flex-col items-start">
        <span className="text-sm font-semibold tracking-tight text-stone-800">
          Sign in with Google
        </span>
        <span className="text-xs text-stone-400">Secure backend session</span>
      </span>
    </button>
  );
}
