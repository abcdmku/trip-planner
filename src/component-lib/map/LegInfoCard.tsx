...
export interface LegInfoCardFeasibility {
  status: FeasibilityStatus;
  color: string;
  label: string;
}

export interface LegInfoCardProps {
  leg: Pick<Leg, 'mode' | 'durationMinutes' | 'distanceMeters' | 'routeType'>;
  fromName: string;
  toName: string;
  onClose: () => void;
  onModeChange?: (mode: TransportMode) => void;
  onRouteTypeChange?: (routeType: RouteType) => void;
  feasibility?: LegInfoCardFeasibility | null;
  openInGoogleMapsUrl?: string | null;
  selectableModes?: readonly { mode: TransportMode; label: string }[];
  routeTypes?: readonly { type: RouteType; label: string }[];
}
...
export function LegInfoCard({
  ...
  selectableModes = SELECTABLE_MODES,
  routeTypes = ROUTE_TYPES,
}: LegInfoCardProps) {
  const currentRouteType = leg.routeType ?? 'directions';

  return (
    <div className="relative w-full max-w-sm rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-lg dark:border-gray-700 dark:bg-gray-800">
      <button
        type="button"
        onClick={onClose}
        className="absolute right-2 top-2 text-gray-400 transition-colors hover:text-gray-600 dark:hover:text-gray-300"
        aria-label="Close"
      >
        <CloseIcon />
      </button>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-800 dark:text-gray-200">
        ...
        <ArrowIcon />
        <span className="max-w-[120px] truncate font-semibold" title={toName}>
          {toName}
        </span>
      </div>
        ...
        <div className="mt-3 border-t border-gray-100 pt-2 dark:border-gray-700">
          ...
          <div className="flex gap-1">
            {routeTypes.map(({ type, label }) => (
              ...
              >
                {type === 'directions' ? <RouteIcon /> : <StraightLineIcon />}
                <span>{label}</span>
              </button>
            ))}
          </div>
...
function ArrowIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="h-3.5 w-3.5 flex-shrink-0 text-gray-400">
      <path
        fillRule="evenodd"
