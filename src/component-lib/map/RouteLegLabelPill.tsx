...
export interface RouteLegLabelPillProps {
  mode: TransportMode;
...
export function RouteLegLabelPill({
  mode,
  durationMinutes,
  color,
...
}: RouteLegLabelPillProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const visibleModes =
    selectableModes === SELECTABLE_MODES
      ? selectableModes.filter(({ mode: selectableMode }) => GOOGLE_DIRECTIONS_MODE_SET.has(selectableMode))
      : selectableModes;
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative' }}>
      <button
        ...
        aria-label={`Travel time ${formatDuration(durationMinutes)}`}
      ...
      >
        {isRecalculating ? <SpinnerIcon /> : <ModeIconSmall mode={mode} />}
      ...
      {isExpanded && !isRecalculating && (
        ...
        >
          {visibleModes.map(({ mode: selectableMode, label }) => (
            <button
              key={selectableMode}
              type="button"
              title={label}
              aria-pressed={selectableMode === mode}
              onClick={(event) => {
                event.stopPropagation();
                handleModeSelect(selectableMode);
              }}
              style={{
                display: 'flex',
                alignItems: 'center',
                ...
                border: 'none',
                cursor: 'pointer',
                backgroundColor: selectableMode === mode ? color : 'transparent',
                color: selectableMode === mode ? '#fff' : '#374151',
                transition: 'background-color 150ms ease',
              }}
            >
              <ModeIconSmall mode={selectableMode} className="h-4 w-4" />
            </button>
          ))}
        </div>
      )}
