...
import { Navbar, type NavbarTheme } from './Navbar';

export type AppShellTab = 'map' | 'itinerary' | 'timeline' | 'optimize';
...
export function AppShell({
  ...
  const desktopMapTabbed =
    desktopLayoutMode !== undefined ? desktopLayoutMode === 'tabbed' : desktopMapTabbedState;
  ...
  const getDesktopResizeMaxWidth = useCallback(() => {
    ...
    return Math.max(LEFT_PANEL_MIN_WIDTH, Math.min(leftPanelMaxByContent, viewportLimit));
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-theme">
      ...
      <MobileTabs activeTab={activeTab} onTabChange={onActiveTabChange} />
    </div>
  );
}
